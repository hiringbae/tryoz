...
function detectClients() {
  ...
  return Object.fromEntries(Object.entries(details).map(([client, detail]) => [client, detail.selected]));
}

function detectClientDetails(cwd = process.cwd()) {
  return {
    codex: detectClient([
      signal("cli", commandExists("codex")),
      signal("config", exists(homePath(".codex")))
    ]),
    claude: detectClient([
      signal("cli", commandExists("claude")),
      signal("config", exists(homePath(".claude")))
    ]),
    cursor: detectClient([
      signal("cli", commandExists("cursor")),
      signal("app", appExists("Cursor")),
      signal("config", exists(homePath(".cursor")))
    ]),
    vscode: detectClient([
      signal("cli", commandExists("code")),
      signal("app", appExists("Visual Studio Code")),
      signal("config", exists(vscodeUserDir()))
    ]),
    cline: detectClient([
      signal("cli", commandExists("cline")),
      signal("config", exists(homePath(".cline")) || exists(clineExtensionConfigPath()))
    ]),
    windsurf: detectClient([
      signal("cli", commandExists("windsurf")),
      signal("app", appExists("Windsurf")),
      signal("config", exists(homePath(".codeium", "windsurf")))
    ]),
    opencode: detectClient([
      signal("cli", commandExists("opencode")),
      signal("config", exists(opencodeConfigPath("global")))
    ]),
    copilot: detectClient([
      signal("cli", commandExists("copilot")),
      signal("config", exists(githubCopilotConfigPath("global")))
    ]),
    "copilot-agent": detectClient([
      signal("project", exists(path.join(cwd, ".github")))
    ]),
    grok: detectClient([
      signal("cli", commandExists("grok") || commandExists("grok-code")),
      signal("config", exists(homePath(".grok")))
    ]),
    gemini: detectClient([
      signal("cli", commandExists("gemini")),
      signal("project", exists(path.join(cwd, "GEMINI.md"))),
      signal("config", exists(homePath(".gemini")))
    ])
  };
}

function signal(source, ok) {
  return { source, ok };
}

function detectClient(signals) {
  const found = signals.find((item) => item.ok);
  if (!found) return { available: false, selected: false, source: "missing", label: "not detected" };
  return {
    available: true,
    selected: found.source !== "config",
    source: found.source,
    label: detectionLabel(found.source)
  };
}

function detectionLabel(source) {
  if (source === "cli") return "CLI detected";
  if (source === "app") return "app detected";
  if (source === "project") return "project config";
  if (source === "config") return "config found";
  return "not detected";
}
...
function writeCodexConfig(context, changes, label) {
  ...
  const nextBlock = [
    ...
    "enabled = true",
    "",
    "[mcp_servers.oz.http_headers]",
    `Authorization = ${tomlString(`Bearer ${context.apiKey}`)}`,
    `"X-Oz-Client" = "codex"`,
    `"X-Oz-SDK-Version" = ${tomlString(SDK_VERSION)}`
  ].join("\n");
...
function setupClaude(context, changes) {
  if (context.scope === "project") {
    upsertJSONServer(path.join(context.cwd, ".mcp.json"), ["mcpServers"], MCP_NAME, httpServer(context, "claude"), context, changes, "claude-project-mcp");
  ...
  if (commandExists("claude") && !context.dryRun) {
    ...
    run("claude", ["mcp", "add-json", "--scope", "user", MCP_NAME, JSON.stringify(httpServer(context, "claude"))]);
...
function setupCursor(context, changes) {
  ...
  upsertJSONServer(configPath, ["mcpServers"], MCP_NAME, {
    ...
    headers: authHeaders(context, "cursor")
...
function setupVSCode(context, changes) {
  ...
  upsertJSONServer(configPath, ["servers"], MCP_NAME, {
    type: "http",
    url: context.endpoint,
    headers: authHeaders(context, "vscode")
...
function setupCline(context, changes) {
  ...
  upsertJSONServer(configPath, ["mcpServers"], MCP_NAME, {
    type: "http",
    url: context.endpoint,
    headers: authHeaders(context, "cline")
...
function setupWindsurf(context, changes) {
  ...
  upsertJSONServer(configPath, ["mcpServers"], MCP_NAME, {
    ...
    headers: authHeaders(context, "windsurf")
...
function setupOpenCode(context, changes) {
  ...
  upsertJSONServer(configPath, ["mcp"], MCP_NAME, {
    ...
    url: context.endpoint,
    enabled: true,
    headers: authHeaders(context, "opencode")
...
function setupCopilot(context, changes) {
  ...
  upsertJSONServer(configPath, ["servers"], MCP_NAME, {
    type: "http",
    url: context.endpoint,
    headers: authHeaders(context, "github-copilot")
...
function setupCopilotAgent(context, changes) {
  upsertJSONServer(path.join(context.cwd, ".github", "mcp.json"), ["servers"], MCP_NAME, {
    type: "http",
    url: context.endpoint,
    headers: authHeaders(context, "github-copilot-agent")
...
function setupGrok(context, changes) {
  ...
  upsertJSONServer(configPath, ["mcpServers"], MCP_NAME, httpServer(context, "grok"), context, changes, "grok");
...
function removeTomlTable(content, tableName) {
  ...
  const childHeaderPrefix = `[${tableName}.`;
  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed === header || trimmed.startsWith(childHeaderPrefix)) {
      skipping = true;
      continue;
    }
    if (skipping && /^\s*\[[^\]]+\]\s*$/.test(line)) {
      if (trimmed.startsWith(childHeaderPrefix)) {
        continue;
      }
      skipping = false;
    }
...
function httpServer(context, client) {
  return {
    type: "http",
    url: context.endpoint,
    headers: authHeaders(context, client)
  };
}

function authHeaders(context, client = "unknown") {
  return {
...
function appExists(appName) {
  ...
  return exists(path.join("/Applications", `${appName}.app`)) || exists(homePath("Applications", `${appName}.app`));
}
